...
GrStringWidthGet(const tContext *pContext, const char *pcString, int32_t lLength)
{
    ...
    // unlikely to ever be passed a string this int32_t.
    //
...
GrFontGlyphRender(const tContext *pContext, const unsigned char *pucData,
                  ...
                  uint32_t bOpaque)
{
    int32_t lIdx, lX0, lY0, lCount, lOff, lOn, lBit, lClipX1, lClipX2;
...
uint32_t
GrFontBlockCodepointsGet(const tFont *pFont, uint16_t usBlockIndex,
                         uint32_t *pulStart)
{
    ASSERT(pFont);
    ASSERT(pulStart);
...
//*****************************************************************************
int32_t
GrStringCodepageSet(tContext *pContext, uint16_t usCodepage)
{
    ASSERT(pContext);
...
static const uint16_t *g_pusLanguageTable;
static const unsigned char *g_pucStringData;

static uint16_t g_usLanguage;
static uint16_t g_usNumLanguages;
static uint16_t g_usNumStrings;
...
GrStringTableSet(const void *pvTable)
{
    ...
    //
    g_usNumStrings = ((uint16_t *)pvTable)[0];
    g_usNumLanguages = ((uint16_t *)pvTable)[1];

    //
    // Save a pointer to the Language Identifier table.
    //
    g_pusLanguageTable = (uint16_t *)pvTable + 2;
...
//*****************************************************************************
uint32_t
GrStringLanguageSet(uint16_t usLangID)
{
    int32_t lLang;
...
//*****************************************************************************
uint32_t
GrStringGet(int32_t lIndex, char *pcData, uint32_t ulSize)
{
    uint32_t ulLen, ulOffset, ulSubCode[16];
    int32_t lPos, lIdx, lBit, lSkip, lBuf;
